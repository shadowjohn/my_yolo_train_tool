...
def download_authorized_youtube_video(url, output_dir, max_duration_seconds=300):
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            if info.get("is_live"):
                raise RuntimeError("不支援直播影片，請改用已授權的短影片或螢幕框選模式。")
            duration = info.get("duration")
            if duration is None:
                raise RuntimeError("無法確認影片長度，為避免誤抓過長內容，請改用螢幕框選模式。")
            duration = int(duration or 0)
            if duration <= 0:
                raise RuntimeError("無法確認影片長度，請改用螢幕框選模式。")
        ...
        raise RuntimeError("YouTube URL 下載失敗，請確認影片可合法存取，或改用螢幕框選模式。") from ex
    if not os.path.isfile(filepath):
        raise RuntimeError("YouTube 下載完成後找不到影片檔，請改用螢幕框選模式。")
    return filepath, info


def iter_video_frames(video_path, fps_target=15, max_duration_seconds=300):
    if fps_target <= 0:
        raise ValueError("fps_target must be greater than zero")
    ...
    try:
        ...
        while True:
            ...
            if time_ms <= 0 and frame_index > 0:
                time_ms = int(round((float(frame_index) / float(source_fps)) * 1000))
