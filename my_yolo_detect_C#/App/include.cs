...
namespace utility
{
    public class myinclude
    {
        ...
        public string json_encode(object input, bool isUtf8 = false)
        {
            if (isUtf8 == false)
            {
                return EscapeUnicode(JsonConvert.SerializeObject(input, Newtonsoft.Json.Formatting.None));
            }
            else
            {
                return json_format(EscapeUnicode(JsonConvert.SerializeObject(input, Newtonsoft.Json.Formatting.None)));
            }
        }
        public string json_format(string input)
        {
            JArray jdod = json_decode(input);
            return EscapeUnicode(JsonConvert.SerializeObject(jdod, Newtonsoft.Json.Formatting.Indented));
        }
        public string json_format_utf8(string input)
        {
            JArray jdod = json_decode(input);
            return JsonConvert.SerializeObject(jdod, Newtonsoft.Json.Formatting.Indented);
        }
